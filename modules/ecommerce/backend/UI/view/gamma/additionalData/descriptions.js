Ext.define('App.modules.ecommerce.backend.UI.view.gamma.additionalData.descriptions', {
    
    alias: 'widget.ecommerce_gamma_additionaldata_descriptions',
    explotation: 'Descriptions for ecommerce gamma (Additional data)',
    config: null,
    ...
    getTitlesFieldset:  function()
    {
        ...
        var ret = 
        {
            ...
            [   
                {
                    xtype: 'label',
                    itemId: module_id + '_gamma_additionaldata_descriptions_title_msg_no_available_lang',
                ...
                {
                    xtype: 'tabpanel',   
                    itemId: module_id + '_gamma_additionaldata_descriptions_tabpanel_title',
                    listeners: {
    ...
    getDescriptionsFieldset: function()
    {
        var me = this;
        var module_id = me.config.module_id;
        var tabItemId = module_id + '_gamma_additionaldata_descriptions_tabpanel_description'; 
        ...
        var ret = 
        {
            ...
            [   
                {
                    xtype: 'label',
                    itemId: module_id + '_gamma_additionaldata_descriptions_description_msg_no_available_lang',
                    text: me.trans('no_available_language'),
                    style: {
                        color: 'red'
                    },
    ...
    createTabContent: function(config, type)
    {
        ...
        var tab = Ext.ComponentQuery.query('#' + module_id + '_gamma_additionaldata_descriptions_tabpanel_' + type)[0];
        var langs = App.app.getController('App.core.backend.UI.controller.common').getAvailableLangs();
        
        if (Ext.isEmpty(langs))
        {
            tab.hide();
            var label = Ext.ComponentQuery.query('#' + module_id + '_gamma_additionaldata_descriptions_' + type + '_msg_no_available_lang')[0];     
            label.show();
        }
        else
        {
