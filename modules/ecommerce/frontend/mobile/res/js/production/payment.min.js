...
var paypal_data=null;var trigger={};$(function(){initClickToPayTooltip();initPaypalButton();$(document).ready(function(){if(payment_way.length===0){showHideSumitButtons()}else{onClickPaymentWay(payment_way)}initValidator("card");initValidator("clicktopay");initValidator("iupay");if(msg_payment_failed_msg.length>0){ecommerce.showBasicDialog(msg_payment_failed_title,msg_payment_failed_msg)}})});function initClickToPayTooltip(){if(is_mobile){return}var c=$("#payment-paymentway-clicktopay-moreinfo");if(!c){return}var b=c.attr("_content");if(!b){return}var a=$(b);c.tooltipster({content:a,theme:"login-tooltip-theme",interactive:true,arrow:true,position:"bottom",offsetY:-11,interactiveTolerance:5,speed:0,trigger:(is_touch_device?"click":"hover")})}function initValidator(b){var a=$("#payment-submit-"+b+"-form");if(!a){return}a.validate({rules:{},messages:{},submitHandler:function(d){var c=$("input[name=paymentway]:checked").val();if(!c){ecommerce.showBasicDialog(msg_payment_ways,msg_select_payment_way);return}ecommerce.setMask(msg_processing_your_order);$(trigger).on("onValidated",function(f,g,e){$(trigger).off("onValidated");if(!g||!e.success){return}d.submit()});validate()}})}function initPaypalButton(){if(typeof paypal=="undefined"){return}$("#payment-submit-paypal-button").hide();paypal.Button.render({env:paypal_env,locale:paypal_locale,style:{size:"medium",color:"gold",shape:"pill"},client:{sandbox:"AaMyA02MkgeUQwggA3wIiTI8965eSmRVgzzwGD7I0dD26lpJs042DMVAxVJ2huh9uMEukjF1pSganiaD",production:"Af1Sm6BWD10GnaCS1SUtcPCpIJcKFQk0zXELrhI-AiPACHTFRhQWf0zP6--MpopuRh6EM-KSb5m-Tyho"},payment:function(d,c){var b=this.props.env;var a=this.props.client;return paypal.rest.payment.create(b,a,{transactions:[{amount:{total:paypal_total_price,currency:"EUR"}}]})},commit:true,onAuthorize:function(a,b){paypal_data=a;$(trigger).on("onValidated",function(d,e,c){$(trigger).off("onValidated");if(!e||!c.success){return}return b.payment.execute().then(function(){ordering()})});validate()},onCancel:function(a,b){}},"#payment-submit-paypal-button")}function onClickPaymentWay(b){payment_way=b;var c=$("#payment-paymentway-"+payment_way);var a=$("#payment-paymentway-"+payment_way+"-radio");$("#payment-paymentway-card").removeClass("payment-paymentway-selected");$("#payment-paymentway-clicktopay").removeClass("payment-paymentway-selected");$("#payment-paymentway-iupay").removeClass("payment-paymentway-selected");$("#payment-paymentway-paypal").removeClass("payment-paymentway-selected");$("#payment-paymentway-fake").removeClass("payment-paymentway-selected");c.addClass("payment-paymentway-selected");a.prop("checked",true);showHideSumitButtons()}function showHideSumitButtons(){var a=$("#payment-submit-card-button");var c=$("#payment-submit-clicktopay-button");var b=$("#payment-submit-iupay-button");var e=$("#payment-submit-paypal-button");var d=$("#payment-submit-fake-button");a.hide();if(c){c.hide()}b.hide();e.hide();d.hide();if(payment_way==="clicktopay"){c.show()}else{if(payment_way==="iupay"){b.show()}else{if(payment_way==="paypal"){e.show()}else{if(payment_way==="fake"){d.show()}else{a.show()}}}}}function validate(){ecommerce.setMask(msg_processing_your_order);var a="controller=modules\\ecommerce\\frontend\\controller\\webpages\\payment&method=validate&paymentWay="+payment_way+"";if(payment_way==="paypal"){a+="&paypalPayerId="+paypal_data.payerID+"&paypalPaymentId="+paypal_data.paymentID+"&paypalPaymentToken="+paypal_data.paymentToken+""}$.ajax({type:"POST",url:"index.php",data:a,success:function(b){b=JSON.parse(b);if(!b.success){$.unblockUI();ecommerce.showBasicDialog(msg_attention+"!",b.msg,b.redirectUrl);$(trigger).trigger("onValidated",[false,b]);return}$(trigger).trigger("onValidated",[true,b])}})}function ordering(){ecommerce.setMask(msg_processing_your_order);var a="controller=modules\\ecommerce\\frontend\\controller\\webpages\\payment&method=ordering";$.ajax({type:"POST",url:"index.php",data:a,success:function(b){b=JSON.parse(b);if(!b.success){$.unblockUI();ecommerce.showBasicDialog(msg_attention+"!",b.msg,b.redirectUrl);return}window.location.href=b.redirectUrl}})}function onFakeSubmit(){$(trigger).on("onValidated",function(b,c,a){$(trigger).off("onValidated");if(!c||!a.success){return}ordering()});validate()};
