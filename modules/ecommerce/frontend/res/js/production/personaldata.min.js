...
$(function(){$("[name=country]").prop("disabled",true);$(document).ready(function(){var a=$("#personal-data-wrapper").validate({event:"blur",rules:{firstName:{required:true},lastName:{required:true},email:{required:true,email:true},phone:{required:true,number:true},address:{required:true},postalCode:{required:true,number:true},city:{required:true},country:{required:true}},messages:{firstName:msg_required_field,lastName:msg_required_field,email:{required:msg_required_field,email:msg_invalid_email},phone:{required:msg_required_field,number:msg_please_enter_numeric_value_without_spaces},address:msg_required_field,postalCode:{required:msg_required_field,number:msg_please_enter_numeric_value_without_spaces},city:msg_required_field,country:msg_required_field},highlight:function(b){$(b).addClass("error")},unhighlight:function(b){$(b).removeClass("error")},debug:false,errorClass:"error",submitHandler:function(d){var b="";if($("[name=company]").val()){b=$("[name=company]").val()}var c="controller=modules\\ecommerce\\frontend\\controller\\webpages\\personaldata&method=validate&firstname="+$("[name=firstName]").val()+"&lastname="+$("[name=lastName]").val()+"&email="+$("[name=email]").val()+"&phone="+$("[name=phone]").val()+"&company="+b+"&address="+$("[name=address]").val()+"&postalcode="+$("[name=postalCode]").val()+"&city="+$("[name=city]").val()+"&country="+$("[name=country]").val()+"&comments="+$("[name=comments]").val()+"";$.ajax({type:"POST",url:"index.php",data:c,success:function(e){e=JSON.parse(e);if(!e.success){if(!e.redirectUrl){alert(e.msg);return false}else{alert(e.msg)}}window.location.href=e.redirectUrl}})}})})});
