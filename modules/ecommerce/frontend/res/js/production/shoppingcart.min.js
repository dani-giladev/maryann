...
$(function(){initSpinners();$("#shoppingcart-content-voucher-tooltip").hide();var a="ontouchstart" in document.documentElement?"touchstart":"click";$(document).on(a,"#shoppingcart-content-voucher-tooltip-button",function(){confirmVoucher()});$(document).ready(function(){})});function initSpinners(){if(is_mobile){return}$("input[name=shoppingcart-content-articles-table-column-amount-content-spinner-name]").each(function(){var e=$(this).attr("_article_code");var d=$(this).attr("_article_title");var b=($(this).attr("_enable")=="1");var g=parseInt($(this).attr("_stock"));var c=parseInt($(this).attr("_min"));var a=parseInt($(this).attr("_max"));$(this).spinner({min:c,max:a+1,step:1,change:function(i,k){var j=$(this).spinner("value");var h=$(this).attr("last_value");if(j<c){j=c;$(this).spinner("value",j)}else{if(j>a){j=a;$(this).spinner("value",j);ecommerce.showDialogWhenAddToCartWithSpinner(d,b,g,(g-a))}}if(j!=h){onChangeArticleAmount(e,j);$(this).attr("last_value",j)}},spin:function(i,k){var j=k.value;if(j>a){j=a;$(this).spinner("value",j);i.preventDefault();ecommerce.showDialogWhenAddToCartWithSpinner(d,b,g,(g-a))}var h=$(this).attr("last_value");if(j!=h){onChangeArticleAmount(e,j);$(this).attr("last_value",j)}}});var f=$(this).spinner("value");$(this).attr("last_value",f)})}function showVoucherDialog(){if($("#shoppingcart-content-voucher-tooltip").is(":visible")){$("#shoppingcart-content-voucher-tooltip").hide()}else{$("#shoppingcart-content-voucher-tooltip").show()}}function initVoucherTooltip(){if(is_mobile){return}var c=$("#shoppingcart-content-voucher");if(!c){return}var b=c.attr("_content");if(!b){return}var a=$(b);c.tooltipster({content:a,theme:"login-tooltip-theme",interactive:true,delay:0,arrow:true,position:"bottom",offsetY:-3,interactiveTolerance:5,speed:0,trigger:"click",autoClose:true,functionReady:function(){$(".tooltipster-default .close").on("click",function(d){d.preventDefault();$("body").click()})}})}function confirmVoucher(){var b=$("#shoppingcart-content-voucher-tooltip-input").val();var a="controller="+ecommerce.getBaseController()+"\\shoppingcart&method=confirmVoucher&vouchercode="+b;$.ajax({type:"POST",url:"index.php",data:a,success:function(c){c=JSON.parse(c);if(!c.success){$("#shoppingcart-content-voucher-tooltip-error-msg").html(c.msg);return false}if(!c.htmlMsg){window.location.reload()}else{$.fancybox({content:c.htmlMsg,closeEffect:"none",closeBtn:true,beforeClose:function(){window.location.reload()}})}}})}function removeFromShoppingcart(b){var a="controller="+ecommerce.getBaseController()+"\\shoppingcart&method=removeFromShoppingcart&code="+b;$.ajax({type:"GET",url:"index.php",data:a,success:function(c){if(is_mobile){window.location.reload();return}c=JSON.parse(c);$("#shoppingcart-content").html(c.content);$("#shoppingcart-menu-option-amount-wrapper").html(c.shoppingcartAmount);$("#shoppingcart-menu-option-totalprice-wrapper").html(c.shoppingcartTotalPrice);$("#shoppingcart-menu-option").tooltipster("content",$(c.shoppingcartTooltip));initSpinners()}})}function removeAllFromShoppingcart(){var a="controller="+ecommerce.getBaseController()+"\\shoppingcart&method=removeAllFromShoppingcart";$.ajax({type:"GET",url:"index.php",data:a,success:function(b){if(is_mobile){window.location.reload();return}b=JSON.parse(b);$("#shoppingcart-content").html(b.content);$("#shoppingcart-menu-option-amount-wrapper").html(b.shoppingcartAmount);$("#shoppingcart-menu-option-totalprice-wrapper").html(b.shoppingcartTotalPrice);$("#shoppingcart-menu-option").tooltipster("content",$(b.shoppingcartTooltip));initSpinners()}})}function onChangeArticleAmount(c,b){if(is_mobile){return}var a="controller="+ecommerce.getBaseController()+"\\shoppingcart&method=onChangeArticleAmount&code="+c+"&amount="+b;$.ajax({type:"GET",url:"index.php",data:a,success:function(d){d=JSON.parse(d);$("#shoppingcart-content-articles-table-column-price-content-price-"+c).html(d.totalArticlePrice);$("#shoppingcart-content-totals-container").html(d.totalPrice);$("#shoppingcart-menu-option-amount-wrapper").html(d.shoppingcartAmount);$("#shoppingcart-menu-option-totalprice-wrapper").html(d.shoppingcartTotalPrice);$("#shoppingcart-menu-option").tooltipster("content",$(d.shoppingcartTooltip))}})}function validate(){ecommerce.setMask(msg_wait_please);var a="controller="+ecommerce.getBaseController()+"\\shoppingcart&method=validate";$.ajax({type:"POST",url:"index.php",data:a,success:function(b){b=JSON.parse(b);if(!b.success){$.unblockUI();ecommerce.showBasicDialog(msg_attention+"!",b.msg,b.redirectUrl);return false}window.location.href=b.redirectUrl}})};
