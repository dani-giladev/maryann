...
$(function(){$(".jcarousel-type1").jcarousel();$(".jcarousel-type1-control-prev").on("jcarouselcontrol:active",function(){$(this).removeClass("inactive")}).on("jcarouselcontrol:inactive",function(){$(this).addClass("inactive")}).jcarouselControl({target:"-=1"});$(".jcarousel-type1-control-next").on("jcarouselcontrol:active",function(){$(this).removeClass("inactive")}).on("jcarouselcontrol:inactive",function(){$(this).addClass("inactive")}).on("click",function(a){a.preventDefault()}).jcarouselControl({target:"+=1"});$(".jcarousel-type1-pagination").on("jcarouselpagination:active","a",function(){$(this).addClass("active")}).on("jcarouselpagination:inactive","a",function(){$(this).removeClass("active")}).on("click",function(a){a.preventDefault()}).jcarouselPagination({item:function(a){return'<a href="#'+a+'">'+a+"</a>"}})});
...
var showcase={scroll_position_timer:null,setStyles:function(){if(is_mobile){return}var d=$(".page-center").width();var a=$("#showcase-sidebar").width();var c=d-a-50;$("#showcase-content").css({width:c+"px"});var b=$(".showcase-content-article").width();$(".showcase-content-article-img-wrapper").css({width:b+"px",height:b+"px"});if($(".showcase-content-article-wrapper").is(".fadeout")){$(".showcase-content-article-wrapper").removeClass("fadeout").addClass("fadein")}},sendSearcherMsg:function(){ecommerce.setMask(msg_wait_please);var c=$("#showcase-content-searchresult-form-text").val();var a=$("#showcase-content-searchresult-form-email").val();var b="controller="+ecommerce.getBaseController()+"\\showcase&method=sendSearcherMsg&msg="+c+"&email="+a;$.ajax({type:"POST",url:"index.php",data:b,success:function(d){$.unblockUI();d=JSON.parse(d);if(!d.success){$("#showcase-content-searchresult-form-error-msg").html(d.msg);return false}$.fancybox({content:d.htmlMsg,closeEffect:"none",closeBtn:true,beforeClose:function(){window.location.reload()}})}})}};$(function(){$(window).resize(function(){showcase.setStyles()});showcase.setStyles();$(".fancybox").fancybox();initMenus();initSliders();initSpinners();initAddButtons();if(is_mobile){$("html, body").animate({scrollTop:scroll_position},800,function(){})}else{$(window).scrollTop(scroll_position)}$(document).ready(function(){$(window).scroll(function(){clearInterval(showcase.scroll_position_timer);showcase.scroll_position_timer=setTimeout("saveScrollPosition()",500)})})});function initMenus(){if(is_mobile){return}$("#showcase-sidebar-categories-menu").menu();$("#showcase-sidebar-brands-combo").menu();initContentMenus()}function initContentMenus(){if(is_mobile){return}$("#showcase-content-menu-sortby-combo").menu();$("#showcase-content-footer-articlesperpage-combo").menu()}function initSliders(){if(is_mobile){return}if(!$("#showcase-sidebar-slider-price-range")){return}var b=$("#showcase-sidebar-slider-price-range");var d=parseFloat(b.attr("_min"));var a=parseFloat(b.attr("_max"));var c=parseFloat(b.attr("_min_value"));var e=parseFloat(b.attr("_max_value"));b.slider({range:true,min:d,max:a,values:[c,e],slide:function(f,g){$("#showcase-sidebar-price-range-filter-value").html(g.values[0]+"\u20ac - "+g.values[1]+"\u20ac")},change:function(f,g){setContent()}});$("#showcase-sidebar-price-range-filter-value").html(b.slider("values",0)+"\u20ac - "+$("#showcase-sidebar-slider-price-range").slider("values",1)+"\u20ac")}function initSpinners(){if(is_mobile){return}$("input[name=showcase-content-article-addtocart-amount-spinner-name]").each(function(){initSpinner($(this))})}function initSpinner(e){var d=e.attr("_article_title");var b=(e.attr("_enable")=="1");var f=parseInt(e.attr("_stock"));var c=parseInt(e.attr("_min"));var a=parseInt(e.attr("_max"));e.spinner({min:c,max:a+1,step:1,change:function(g,i){var h=e.spinner("value");if(h<c){e.spinner("value",c)}else{if(h>a){e.spinner("value",a);ecommerce.showDialogWhenAddToCartWithSpinner(d,b,f,a)}}},spin:function(g,i){var h=i.value;if(h>a){e.spinner("value",a);g.preventDefault();ecommerce.showDialogWhenAddToCartWithSpinner(d,b,f,a)}}})}function initAddButtons(){if(is_mobile){return}$("button[name=showcase-content-article-addtocart-button-name]").each(function(){initAddButton($(this))})}function initAddButton(b){var a=b.attr("_disabled");if(a){b.prop("disabled",true);b.removeClass("button-addtocart").addClass("button-disabled")}}function onClickOutstandingArticlesCheckboxFilter(){setContent()}function onClickNoveltyArticlesCheckboxFilter(){setContent()}function onClickPackArticlesCheckboxFilter(){setContent()}function onClickChristmasArticlesCheckboxFilter(){setContent()}function onClickCategoriesCheckboxFilter(){setContent()}function onClickBrandsCheckboxFilter(){setContent()}function onClickGammaCheckboxFilter(){setContent()}function onClickArticlePropertiesCheckboxFilter(){setContent()}function onChangeArticlesPerPage(){setContent()}function onChangeSortby(){setContent()}function setContent(){var g,d;var i="";var e="";if($("#showcase-sidebar-slider-price-range").is(":visible")){i=$("#showcase-sidebar-slider-price-range").slider("values",0);e=$("#showcase-sidebar-slider-price-range").slider("values",1)}var h=false;if($("#showcase-sidebar-outstanding-filter")){h=$("#showcase-sidebar-outstanding-filter").prop("checked")}var m=false;if($("#showcase-sidebar-novelty-filter")){m=$("#showcase-sidebar-novelty-filter").prop("checked")}var l=false;if($("#showcase-sidebar-pack-filter")){l=$("#showcase-sidebar-pack-filter").prop("checked")}var n=false;if($("#showcase-sidebar-christmas-filter")){n=$("#showcase-sidebar-christmas-filter").prop("checked")}g=[];$("input[name=showcase-sidebar-categories-filter]").each(function(){if($(this).prop("checked")){g.push($(this).attr("code"))}});var f=JSON.stringify(g);g=[];$("input[name=showcase-sidebar-brands-filter]").each(function(){if($(this).prop("checked")){g.push($(this).attr("code"))}});var o=JSON.stringify(g);g=[];$("input[name=showcase-sidebar-gamma-filter]").each(function(){if($(this).prop("checked")){g.push($(this).attr("code"))}});var b=JSON.stringify(g);d={};$("input[name=showcase-sidebar-articleproperties-filter]").each(function(){if($(this).prop("checked")){var q=$(this).attr("code");var p=$(this).attr("amount");var r=$(this).attr("value_code");if(!d[q]){d[q]={}}if(!d[q]["amounts"]){d[q]["amounts"]={}}if(!d[q]["amounts"][p]){d[q]["amounts"][p]={}}if(!d[q]["amounts"][p]["values"]){d[q]["amounts"][p]["values"]={}}d[q]["amounts"][p]["values"][r]={}}});var c=JSON.stringify(d);var a="";if($("#showcase-content-footer-articlesperpage-combo").is(":visible")){a=$("#showcase-content-footer-articlesperpage-combo").val()}var j="";if($("#showcase-content-menu-sortby-combo").is(":visible")){j=$("#showcase-content-menu-sortby-combo").val()}var k="controller="+ecommerce.getBaseController()+"\\showcase&method=sendContentToClient&min_price="+i+"&max_price="+e+"&outstanding="+h+"&novelty="+m+"&pack="+l+"&christmas="+n+"&categories="+f+"&brands="+o+"&gamma="+b+"&article_properties="+c+"&articlesperpage="+a+"&sortby="+j;$.ajax({type:"GET",url:"index.php",data:k,success:function(p){p=JSON.parse(p);$("#showcase-content").html(p.articlesContent);showcase.setStyles();initContentMenus();initSpinners();initAddButtons()}})}function addToShoppingcart(c){ecommerce.setMask(msg_processing_your_request);var b=$("input[name=showcase-content-article-addtocart-amount-spinner-name][_article_code='"+c+"']").val();var a="controller="+ecommerce.getBaseController()+"\\showcase&method=addToShoppingcart&code="+c+"&amount="+b;$.ajax({type:"GET",url:"index.php",data:a,success:function(d){$.unblockUI();d=JSON.parse(d);$("#shoppingcart-menu-option-amount-wrapper").html(d.shoppingcartAmount);$("#shoppingcart-menu-option-totalprice-wrapper").html(d.shoppingcartTotalPrice);$("#shoppingcart-menu-option").tooltipster("content",$(d.shoppingcartTooltip));$.fancybox({content:d.windowAfterAddToShoppingcart,closeEffect:"none",closeBtn:false});$("#showcase-content-article-addtocart-article-"+c).html(d.addToShoppingcartWidgetsContent);initSpinner($("input[name=showcase-content-article-addtocart-amount-spinner-name][_article_code="+c+"]"));initAddButton($("button[name=showcase-content-article-addtocart-button-name][_article_code="+c+"]"))}})}function saveScrollPosition(){var a=$(window).scrollTop();if(a==scroll_position){return}var b="controller="+ecommerce.getBaseController()+"\\showcase&method=saveScrollPosition&position="+a;$.ajax({type:"GET",url:"index.php",data:b,success:function(c){}})};
