...
$(function(){$(document).ready(function(){var a=$("#signin-wrapper").validate({event:"blur",rules:{firstName:{required:true},lastName:{required:true},email:{required:true,email:true},password:{required:true,minlength:5},confirmPassword:{required:true,minlength:5,equalTo:"#signin-field-password"},captcha:{required:true}},messages:{firstName:msg_required_field,lastName:msg_required_field,email:{required:msg_required_field,email:msg_invalid_email},password:{required:msg_required_field,minlength:msg_password_too_short},confirmPassword:{required:msg_required_field,minlength:msg_password_too_short,equalTo:msg_please_enter_same_value},captcha:msg_required_field},highlight:function(b){$(b).addClass("error")},unhighlight:function(b){$(b).removeClass("error")},debug:false,errorClass:"error",submitHandler:function(c){ecommerce.setMask(msg_processing_your_request);var b="controller=modules\\ecommerce\\frontend\\controller\\webpages\\signin&method=validate&firstname="+$("#signin-field-firstname").val()+"&lastname="+$("#signin-field-lastname").val()+"&email="+$("#signin-field-email").val()+"&password="+$("#signin-field-password").val()+"&confirmpassword="+$("#signin-field-confirm-password").val()+"&captcha="+$("#signin-field-captcha").val()+"&newsletters="+$("#signin-field-newsletters").prop("checked")+"";$.ajax({type:"POST",url:"index.php",data:b,success:function(d){$.unblockUI();d=JSON.parse(d);if(!d.success){var e=d.fieldMsg;if(e==="email"){a.showErrors({email:d.msg})}else{if(e==="confirmPassword"){a.showErrors({confirmPassword:d.msg})}else{if(e==="captcha"){a.showErrors({captcha:d.msg})}}}return false}$.fancybox({content:d.thanksForSigninWindow,closeEffect:"none",closeBtn:true,beforeClose:function(){ecommerce.setMask(msg_wait_please);window.location.href=d.redirectUrl}})}})}})})});function refreshCaptcha(){var a=$("#signin-captcha-img");var b=a.attr("src");document.getElementById("signin-captcha-img").src=b};
