...
var ecommerce={setMask:function(a){$.blockUI({message:a,css:{fontSize:"16px",border:"none",padding:"15px",backgroundColor:"#fff","-webkit-border-radius":"10px","-moz-border-radius":"10px",opacity:0.5,color:"#000"}})},showDialogWhenAddToCartWithSpinner:function(d,a,g,c){var f,e;var b=$("#addtocart-amount-spinner-dialog-warning").dialog({resizable:false,title:msg_attention+"!",modal:true,show:{effect:"blind",duration:300},hide:{effect:"explode",duration:1000},buttons:[{text:msg_accept,click:function(){$(this).dialog("close")}}]});if(a){f=msg_there_is_only+" <b>"+g+"</b> "+msg_units_in_stock+".";e=g-c;if(g>0){if(e===g){f+=" "+msg_and_all_are_in_shoppingcart+"."}else{if(e>0){f+="</br></br>"+msg_you_have_already_added+" "+e+" "+msg_to_shoppingcart+".";f+=" "+msg_you_can_only_add+"  "+c+" "+msg_more+"."}}}}else{f=msg_you_can_add_max_units_in_shoppingcart+".";e=100-c;if(e>0){if(e>=100){f+=" "+msg_and_all_are_in_shoppingcart+"."}else{f+="</br></br>"+msg_you_have_already_added+" "+e+" y ";f+=" "+msg_you_can_only_add.toLowerCase()+"  "+c+" "+msg_more+"."}}}b.html('<div class="addtocart-amount-spinner-dialog-warning-article-title">'+d+'</div><div class="addtocart-amount-spinner-dialog-warning-text">'+f+"</div>")},showBasicDialog:function(d,b,c){var a=$("#basic-dialog").dialog({resizable:false,title:d,modal:true,show:{effect:"blind",duration:300},hide:{effect:"explode",duration:1000},buttons:[{text:msg_accept,click:function(){if(!c){$(this).dialog("close");return}window.location.href=c}}]});a.html('<div class="basic-dialog-content">'+b+"</div>")},resetAll:function(){var a="controller=modules\\ecommerce\\frontend\\controller\\session&method=resetAll";$.ajax({type:"GET",url:"index.php",data:a,success:function(){}})}};

var search_articles_timer=null;var menu={setStyles:function(){var a=$(".page-center").width();if(a>1100){$(".menu-main-firstlevel-text").css({"font-size":"15px","padding-top":"13px"});$(".header-specialmenu").css({"font-size":"14px"})}else{if(a>1000){$(".menu-main-firstlevel-text").css({"font-size":"14px","padding-top":"13px"});$(".header-specialmenu").css({"font-size":"13px"})}else{$(".menu-main-firstlevel-text").css({"font-size":"13px","padding-top":"14px"});$(".header-specialmenu").css({"font-size":"12px"})}}}};$(function(){var c=$("nav");var a=c.offset();var b=c.height();$(window).resize(function(){menu.setStyles()});menu.setStyles();$(window).scroll(function(){var d=$(window).scrollTop();var e=$("#content-wrapper");if(d>a.top){c.addClass("nav-fixed");e.css({"padding-top":b+"px"})}else{c.removeClass("nav-fixed");e.css({"padding-top":0+"px"})}$("[_has_tooltip=true]").each(function(){setTooltipMaxHeight($(this))})});initMainMenu();initSearcher();$(document).ready(function(){})});function initMainMenu(){$("[_has_tooltip=true]").each(function(){if($(this).attr("_is_for_categories")){initTooltipMenuForCategories($(this))}});initTooltipMenuForShoppingcart();initTooltipMenuForSearcher();initTooltipMenuForArticleTextOnBreadcrumbs()}function initTooltipMenuForCategories(f){var d=f.attr("_content");if(!d){return}var c=$(d);var e=f.position().left;var a=e*(-1);var g=0;var b=calculateTooltipMaxHeight(f);c.css("max-height",b+"px");f.tooltipster({content:c,theme:"menu-main-tooltip-theme",interactive:true,animationDuration:0,arrow:false,functionReady:function(){f.addClass("menu-main-firstlevel-hover")},functionAfter:function(){f.removeClass("menu-main-firstlevel-hover")},position:"bottom-left",offsetY:(g-12),offsetX:-1,interactiveTolerance:5,speed:0,trigger:(is_touch_device?"click":"hover")})}function initTooltipMenuForShoppingcart(){var d=$("#shoppingcart-menu-option");var c=d.attr("_content");if(!c){return}var b=$(c);var a=calculateTooltipMaxHeight(d);b.css("max-height",a+"px");d.tooltipster({content:b,theme:"shoppingcart-menu-option-menu-option-tp-theme",interactive:true,arrow:false,position:"bottom-right",offsetY:-12,interactiveTolerance:5,speed:0,trigger:(is_touch_device?"click":"hover")})}function initTooltipMenuForSearcher(){var d=$("#searcher");var c=d.attr("_content");var b;if(c){b=$(c);var a=calculateTooltipMaxHeight(d);b.css("max-height",a+"px")}else{b=null}d.tooltipster({content:b,theme:"shoppingcart-menu-option-menu-option-tp-theme",position:"bottom",offsetY:-18,interactive:true,interactiveTolerance:5,speed:0,trigger:(is_touch_device?"click":"hover"),arrow:false})}function initTooltipMenuForArticleTextOnBreadcrumbs(){var c=$("#menu-breadcrumbs-breadcrumb-article-tooltip");if(!c){return}var b=c.attr("_content");if(!b){return}var a=$(b);c.tooltipster({content:a,theme:"menu-breadcrumbs-breadcrumb-article-tooltip-theme",position:"bottom",offsetY:1,interactive:true,interactiveTolerance:5,speed:0,trigger:(is_touch_device?"click":"hover"),arrow:true})}function setTooltipContent(b,a){b.tooltipster("content",a)}function setTooltipMaxHeight(d){var c=d.attr("_content");if(!c){return}var b=d.tooltipster("content");if(!b){return}var a=calculateTooltipMaxHeight(d);b.css("max-height",a+"px");setTooltipContent(d,b)}function calculateTooltipMaxHeight(d){var c=$(window).scrollTop();var b=d.offset().top;var a=($(window).height()-(b-c))-100;return a}function initSearcher(){$("#searcher-input").keyup(function(a){if(a.keyCode==13){searcher(true);return}clearInterval(search_articles_timer);search_articles_timer=setTimeout("searcher()",500)})}function searcher(e){var d=$("#searcher");var b;var c=$("#searcher-input").val();if(c.length<3){setTooltipContent(d,null);return}if(e){window.location.href=base_url+"/showcase?search="+c;return}var a="controller=modules\\ecommerce\\frontend\\controller\\searcher&method=search&value="+c;$.ajax({type:"GET",url:"index.php",data:a,success:function(f){f=JSON.parse(f);var h=f.searchResult;if(h){b=$(h);var g=calculateTooltipMaxHeight(d);b.css("max-height",g+"px")}else{b=null}setTooltipContent(d,b);d.tooltipster("show")}})};

$(function(){initLoginMenu();initLanguagesMenu()});function initLanguagesMenu(){var c=$("#pre-header-languages");if(!c){return}var b=c.attr("_content");if(!b){return}var a=$(b);c.tooltipster({content:a,theme:"login-tooltip-theme",interactive:true,delay:0,arrow:true,position:"bottom",offsetY:-3,interactiveTolerance:5,speed:0,trigger:"click",autoClose:true,functionReady:function(){$(".tooltipster-default .close").on("click",function(d){d.preventDefault();$("body").click()})}})}function initLoginMenu(){var c=$("#pre-header-menu-login");if(!c){return}var b=c.attr("_content");if(!b){return}var a=$(b);c.tooltipster({content:a,theme:"login-tooltip-theme",interactive:true,delay:0,arrow:true,position:"bottom",offsetY:-3,interactiveTolerance:5,speed:0,trigger:"click",autoClose:true,functionReady:function(){$(".tooltipster-default .close").on("click",function(d){d.preventDefault();$("body").click()})}})}function login(){var b=$("#login-tooltip-loginuser").val();var a=$("#login-tooltip-loginpassword").val();var c="controller=modules\\ecommerce\\frontend\\controller\\webpages\\user&method=login&email="+b+"&password="+a;$.ajax({type:"POST",url:"index.php",data:c,success:function(d){d=JSON.parse(d);if(!d.success){$("#login-tooltip-error-msg").html(d.msg);return false}var e=$("#pre-header-menu-login");e.tooltipster("hide");window.location.reload()}})}function logout(){var a="controller=modules\\ecommerce\\frontend\\controller\\webpages\\user&method=logout";$.ajax({type:"POST",url:"index.php",data:a,success:function(b){b=JSON.parse(b);if(b.redirectUrl){window.location.href=b.redirectUrl}else{window.location.reload()}}})};
...
var current_password="";$(function(){$("#user-field-email").prop("disabled",true);$("#user-field-country").prop("disabled",true);current_password=$("#user-field-password").val();$(document).ready(function(){var a=$("#user-wrapper").validate({event:"blur",rules:{firstName:{required:true},lastName:{required:true},email:{required:true,email:true},password:{required:true,minlength:5},confirmPassword:{required:true,minlength:5,equalTo:"#user-field-password"},phone:{number:true},postalCode:{number:true}},messages:{firstName:msg_required_field,lastName:msg_required_field,email:{required:msg_required_field,email:msg_invalid_email},password:{required:msg_required_field,minlength:msg_password_too_short},confirmPassword:{required:msg_required_field,minlength:msg_password_too_short,equalTo:msg_please_enter_same_value},phone:{number:msg_please_enter_numeric_value_without_spaces},postalCode:{number:msg_please_enter_numeric_value_without_spaces}},highlight:function(b){$(b).addClass("error")},unhighlight:function(b){$(b).removeClass("error")},debug:false,errorClass:"error",submitHandler:function(d){ecommerce.setMask(msg_processing_your_request);var b="";if($("#user-field-company").val()){b=$("#user-field-company").val()}var c="controller=modules\\ecommerce\\frontend\\controller\\webpages\\user&method=save&firstname="+$("#user-field-firstname").val()+"&lastname="+$("#user-field-lastname").val()+"&email="+$("#user-field-email").val()+"&currentpassword="+current_password+"&password="+$("#user-field-password").val()+"&confirmpassword="+$("#user-field-confirm-password").val()+"&phone="+$("#user-field-phone").val()+"&company="+b+"&address="+$("#user-field-address").val()+"&postalcode="+$("#user-field-postalcode").val()+"&city="+$("#user-field-city").val()+"&country="+$("#user-field-country").val()+"&comments="+$("#user-field-comments").val()+"&newsletters="+$("#user-field-newsletters").prop("checked")+"";$.ajax({type:"POST",url:"index.php",data:c,success:function(e){$.unblockUI();e=JSON.parse(e);if(!e.success){var f=e.fieldMsg;if(f==="email"){a.showErrors({email:e.msg});return false}else{if(f==="confirmPassword"){a.showErrors({confirmPassword:e.msg});return false}else{if(f==="currentPassword"){}else{alert(e.msg);return false}}}}$.fancybox({content:e.messageAfterSubmitWindow,closeEffect:"none",closeBtn:true,beforeClose:function(){ecommerce.setMask(msg_wait_please);window.location.href=e.redirectUrl}})}})}})})});
